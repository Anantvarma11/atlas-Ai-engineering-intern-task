...
# Every hotel has a valid match_method
bad = con.execute("SELECT COUNT(*) FROM canonical_hotels WHERE match_method NOT IN ('geo_fuzzy','rescue','llm','singleton')").fetchone()[0]
print(f"  hotels with invalid match_method: {bad}")
if bad: errors.append("invalid match_method")

# near_misses candidate_supplier is always 'a' or 'b'
bad = con.execute("SELECT COUNT(*) FROM near_misses WHERE candidate_supplier NOT IN ('a','b')").fetchone()[0]
print(f"  near_misses with invalid candidate_supplier: {bad}")
if bad: errors.append("invalid near_miss candidate_supplier")
...
sample = data[0]
for key in ["id","name","match_status","match_confidence","match_method","supplier_a_id","supplier_b_id","rooms","near_misses"]:
...
required = ["id","name","address","lat","lon","stars","amenities","image_urls",
            "match_status","match_confidence","match_method","sources","rooms","near_misses"]
