...
def build_canonical(
    ...
    if not matches_df.empty:
        for _, match in matches_df.iterrows():
            ...
            # Near-misses for this A hotel
            if not near_misses_df.empty and "a_id" in near_misses_df.columns:
                nm_for_a = near_misses_df[near_misses_df["a_id"] == a_id]
                for _, nm in nm_for_a.iterrows():
                    near_miss_rows.append(
                        {
                            "canonical_hotel_id": cid,
                            "supplier_b_id":      nm["b_id"],
...
def _write_db(
    ...
    hotel_rows = [
        (
            ...
            h["match_status"], h["match_confidence"],
    ...
    # ── Near-misses ───────────────────────────────────────────────────────────
    nm_rows = [
        (n["canonical_hotel_id"], n["supplier_b_id"], n["confidence"], n["geo_score"], n["name_score"])
